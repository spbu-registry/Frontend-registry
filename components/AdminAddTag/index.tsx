...
export function AdminAddTag (
    {setInput, suggestedTagsList, setTag, onClose} : AdminAddTagProps) {
    ...
    return <div className={styles.PopUp}>
        <div className={styles.Wrapper}>
            ...
            <svg onClick={() => onClose()} className={styles.Close} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><title>close</title><path d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" /></svg>
            ...
            <ul className={styles.TagList}>
                {suggestedTagsList.map((value) => {
                    return <li key={value} onClick={() => setCurrentInput(value)}>
                        {value}
                    </li>
                })}
            </ul>
...
export function TestAdminAddTag () {
    ...
    const setInput = (text : string) => {
        const myTags = ['Ohoooooo', 'LMAO', 'I love myself', 'one piece is real', 'Hey']
        setTagList(myTags.filter((val) => val.includes(text)))
    }

    return !closed ? <AdminAddTag
    setInput={setInput}
    suggestedTagsList={tagList}
    setTag={() => {setClosed(true)}}
