...
export function LinksToPresentations ({linksAndLabels, removeLink} : LinksToPresentationsProps) {
    ...
    style={{'--options' : Math.floor(linksAndLabels.size / 2)} as React.CSSProperties}>
        {
            mapOverMap(linksAndLabels, (label, link) => {
                return <div key={label} className={styles.LinkBox}>
                    <label id={label + link} className={styles.Row}>{label}</label>
                    <div className={styles.Row}>
                        {closeSVG(styles.SVG, () => removeLink(label))}
                        <a
                        className={styles.Link} 
                        aria-labelledby={label + link} 
                        href={link}>презентация команды</a>
                    </div>
                </div>
            })
        }
